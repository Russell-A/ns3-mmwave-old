...
int main (int argc, char *argv[]){
    ...
    // SUMO Configuration
    sumoClient->SetAttribute ("SumoConfigPath", StringValue ("examples/simple-junction/simple-junction/junction.sumo.cfg"));
    ...
    std::function<Ptr<Node>()> setupNewWifiNode = [&]() -> Ptr<Node> {
        if (nodeCounter >= n.GetN())
            NS_FATAL_ERROR("Node Container empty!: " << nodeCounter << " nodes created.");
  ...
  sumoClient->SumoSetup(setupNewWifiNode, shutdownWifiNode);
    ...
    //  AnimationInterface anim ("examples/simple-juction/sumple-junction/ns3-sumo-coupling.xml"); // Mandatory
